...
def parse_args():
    ...
    parser.add_argument(
        "--proxy-domains", dest="user_rule", help="直接通过代理域名的文件，每行一个"
    )
    parser.add_argument(
        "--direct-domains", dest="direct_rule", help="直连的域名文件，每行一个"
    )
    parser.add_argument(
        "--localtld-domains",
        dest="localtld_rule",
    ...
    parser.add_argument(
        "--ip-file", dest="ip_file", required=True, help="中国IP地址段文件"
    )
...
def generate_pac_fast(domains, proxy, direct_domains, cidrs, local_tlds):
    ...
    proxy_content = proxy_content.replace(
        "__DOMAINS__", json.dumps(domains_list, sort_keys=True, separators=(",", ":"))
    )
    ...
    proxy_content = proxy_content.replace("__CIDRS__", cidrs)

    tlds_list = []
    for domain in local_tlds:
        tlds_list.append(domain)
    proxy_content = proxy_content.replace(
        "__LOCAL_TLDS__", json.dumps(tlds_list, sort_keys=True, separators=(",", ":"))
    )
...
def main():
    ...
    pac_content = generate_pac_fast(
        user_rule, args.proxy, direct_rule, cidrs, localtld_rule
    )
